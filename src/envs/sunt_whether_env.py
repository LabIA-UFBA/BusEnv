...
class parallel_env(ParallelEnv):
    ...
    def __init__(self, network: nx.Graph, actions_amount: int, max_steps: int, num_agents=2, agents_per_route=None,
        ...
        # Create metrics to track reward evolution
        self.debug_rewards = {
            "eff_no_rain": [],
            "eff_rain": [],
        }
        ...
        # --- Daily Data Control ---
        self.daily_data_path = "/home/marcel/Documents/NZ/New_Version_BusEnv_LabIA/BusEnv/src/training_observation/daily"  # Path to daily data files
        ...
        # rain info
        with open('/home/marcel/Documents/NZ/New_Version_BusEnv_LabIA/BusEnv/src/training_observation/daily/climate_data/climate_data_2.pkl', 'rb') as f:
            self.climate_data = pickle.load(f)

        # CONFIGURATION
        # Hard coding reward type ('penalization', 'bonus', 'none')
        self.reward_type = 'PROP_BONUS_10AG'
        self.max_episodes = 183
    ...
    def reset(self, seed=None, options=None):
        ...
        for agent in self.agents:
            ...
            # Check if it's raining at this time
            time = self.agent_times[agent]
            date = self.date
            hour_of_day = int(time//3600)
            hour_str = f"{hour_of_day:02d}00 UTC"
            self.date = self.date.replace('-','/')
            row = self.climate_data[
                (self.climate_data['date'] == self.date) &
        ...
        if not hasattr(self, "csv_initialized"):
            import csv
            csv_dir = "/home/marcel/Documents/NZ/New_Version_BusEnv_LabIA/reward_logs"
            i = 1
            csv_name = f"reward_info_{self.reward_type}_{i}.csv"
            ...
            self.csv_writer.writerow([
                "episode", "step", "agent",
                "reward", "eff", "modified_eff",
                "reward_type", "is_raining",
                "date", "hour"
            ])
    ...
    def step(self, actions):
        ...
        for agent in self.possible_agents:
            ...
            # === ACTION 1: MOVE ===
                ...
                reward, eff_dicc = self.reward.getReward(
                    new_state=next_node,
                    ...
                    last_rain_eff=self.last_rain_eff[agent]
                )

                if eff_dicc['is_raining']:
                    self.last_rain_eff[agent] = eff_dicc['last_rain_eff']
                self.estimated_times[agent] = 0.0

                # eff components
                modified_eff = eff_dicc['modified_eff']
                eff = eff_dicc['eff']
                is_raining = eff_dicc['is_raining']
                reward_type = eff_dicc['reward_type']
                agent = agent
    ...
    def load_current_day_data(self):
        ...
        day_number = self.current_day_index + 1
        
        # Define a max executing time
        if day_number >= self.max_episodes:
            if hasattr(self, "csv_file"):
                self.csv_file.close()
                print("💾 Closed reward_debug.csv file.")
            print("⏳ Max iteration reached. Stopping the environment.")
            sys.exit() 
...
class DefaultReward(RewardBaseClass):
    ...
    def getReward(
        ...
        agent_state=None, headways=None, is_raining=False, reward_type=None, last_rain_eff=None
    ):
        ...
        modified_eff = eff
        # Penalization
        if is_raining and reward_type == 'penalization':
            rain_strength = 0.2 # Penalization intensity
            modified_eff = eff * rain_strength * (1-eff) # If it's raining, efficiency is penalized more
            # modified_eff = eff * (1 - rain_strength * (1 - eff))

        # Bonus
        if is_raining and reward_type == 'bonus':
            rain_bonus = 0.2 # Bonus intensity
            modified_eff = np.clip(eff + rain_bonus * eff, 0.0, 1.0) # If it's raining, efficiency is rewarded more

        # Additional reward for relative improvement
        if is_raining:
            rain_bonus = 0.2 if eff > last_rain_eff else 0.0
            last_rain_eff = eff
        else:
            rain_bonus = 0.0
        ...
        reward = (
            -w["occ_penalty"] * occ_pen +
             ...
             w["sync_score"] * sync +
             w["energy_efficiency"]*eff *(2.0 if is_raining else 1.0)  + 
             rain_bonus
        )
       ...
       # weighted_eff = w["energy_efficiency"] * eff
        ...
        eff_dic = {'eff': eff, 'modified_eff': modified_eff, 'is_raining': is_raining, 'reward_type': reward_type, 'last_rain_eff': last_rain_eff}
        return reward, eff_dic
